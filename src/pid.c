...
static float Ki = 2e-5;          // (I)ntegral Tuning Parameter        
static float Kd = 3e-4;          // (D)erivative Tuning Parameter       
float iTerm = 0;       // Used to accumulate error (integral)
float lastTime = 0;    // Records the time the function was last called
static float maxPID = 100;    // The maximum value that can be output

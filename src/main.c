...
int main(void)
{
	...
	float pidMotorOutput;

	double dt;
	bool prevEstimate = false;
	int count = 0;
	...
	while (1) {
		...
		/*
		//Write register address first, then read (low byte, then high byte)
		ret = i2c_write_read_dt(&dev_i2c,&tempLowRegister,1,&temp_reading[0],1);
		if(ret != 0){
			printk("Failed to write/read I2C device address %x at Reg. %x \r\n", dev_i2c.addr, tempLowRegister);
			success = 0;
		}
		
		ret = i2c_write_read_dt(&dev_i2c,&tempHighRegister,1,&temp_reading[1],1);
		if(ret != 0){
			printk("Failed to write/read I2C device address %x at Reg. %x \r\n", dev_i2c.addr, tempHighRegister);
			success = 0;
		}

		trueTemp = (float) (temp_reading[1] * 256 + temp_reading[0]) / 340 + 36.53;
		trueTempF = 1.8 * trueTemp + 32;
		*/
		...
		/*
		ret = i2c_write_read_dt(&dev_i2c,&gyroXLowRegister,1,&gyro_reading[0],1);
		if(ret != 0){
			printk("Failed to write/read I2C device address %x at Reg. %x \r\n", dev_i2c.addr, gyroXLowRegister);
			success = 0;
		}
		ret = i2c_write_read_dt(&dev_i2c,&gyroXHighRegister,1,&gyro_reading[1],1);
		if(ret != 0){
			printk("Failed to write/read I2C device address %x at Reg. %x \r\n", dev_i2c.addr, gyroXHighRegister);
			success = 0;
		}

		ret = i2c_write_read_dt(&dev_i2c,&gyroYLowRegister,1,&gyro_reading[2],1);
		if(ret != 0){
			printk("Failed to write/read I2C device address %x at Reg. %x \r\n", dev_i2c.addr, gyroYLowRegister);
			success = 0;
		}
		ret = i2c_write_read_dt(&dev_i2c,&gyroYHighRegister,1,&gyro_reading[3],1);
		if(ret != 0){
			printk("Failed to write/read I2C device address %x at Reg. %x \r\n", dev_i2c.addr, gyroYHighRegister);
			success = 0;
		}

		ret = i2c_write_read_dt(&dev_i2c,&gyroZLowRegister,1,&gyro_reading[4],1);
		if(ret != 0){
			printk("Failed to write/read I2C device address %x at Reg. %x \r\n", dev_i2c.addr, gyroZLowRegister);
			success = 0;
		}
		ret = i2c_write_read_dt(&dev_i2c,&gyroZHighRegister,1,&gyro_reading[5],1);
		if(ret != 0){
			printk("Failed to write/read I2C device address %x at Reg. %x \r\n", dev_i2c.addr, gyroZHighRegister);
			success = 0;
		}
		*/
		...
		if(success){
			...
			/*if(!prevEstimate){
				pitch = predictPitchAccel(accelX, accelY, accelZ);
				roll = predictRollAccel(accelX, accelY, accelZ);
				prevEstimate = true;
			} else{
				pitch = calculatePitch(gyroX, gyroY, gyroZ, accelX, accelY, accelZ, pitch, roll, dt);
				roll = calculateRoll(gyroX, gyroY, gyroZ, accelX, accelY, accelZ, pitch, roll, dt);
			}*/

			estimate(gyroX, gyroY, gyroZ, accelX, accelY, accelZ, &roll, &pitch, dt, P, state_estimate, count);

			pidMotorOutput = pid(roll * 180 / PI, dt);
			pw = move_continuous_servo(-pidMotorOutput);
			//change_servo_angle(pidMotorOutput);

			//printk("Pitch: %.1lf | Roll: %.1lf | dt: %lf\n", pitch * 180 / PI, roll * 180 / PI, dt);

			//printk("%d\n", success);
			if(current_time - last_time > 100){
				//printk("here");
				//printk("Pitch: %.1f | Roll: %.1f | PID Output: %.1f | dt: %f\n", pitch * 180 / PI, roll * 180 / PI, pidMotorOutput, dt);
				printk("Pitch: %.1lf | Roll: %.1lf | dt: %lf | PID output: %f | Pulse width: %f\n", pitch * 180 / PI, roll * 180 / PI, dt, pidMotorOutput, pw);
				last_time = current_time;
			}
			//printk("%f %f %f %f %f %f %lf %lf\n\n", gyroX, gyroY, gyroZ, accelX, accelY, accelZ, pitch, roll);
			//printk("%f %f %f %f %f %f %lf\n", gyroX, gyroY, gyroZ, accelX, accelY, accelZ, dt);
		}
		//k_msleep(500);
	}
	return 0;
